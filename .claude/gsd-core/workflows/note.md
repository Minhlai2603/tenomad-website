...
- **Global scope**: `D:/gsd-tenomad-blog/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/gsd-tenomad-blog/.claude/notes/`)
...
2. Glob `D:/gsd-tenomad-blog/.claude/notes/*.md` (if directory exists) — global notes
...
Global (D:/gsd-tenomad-blog/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `D:/gsd-tenomad-blog/.claude/notes/` — works in any directory
